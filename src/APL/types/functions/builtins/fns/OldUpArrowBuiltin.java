...
public class OldUpArrowBuiltin extends Builtin implements DimDFn {
  ...
  public static Value on(int[] sh, int[] off, Value w, Callable blame) { // +TODO move to new
    int rank = sh.length;
  ...
  public static Value undo(int[] e, Value w, Value origW, Callable blame) { // +TODO move
    if (e.length==1 && w.rank==1) {
      int am = e[0];
      if (am > 0) return OldCatBuiltin.cat(w, OldUpArrowBuiltin.on(new int[]{origW.ia-am}, e, origW, blame), 0, blame);
      else return OldCatBuiltin.cat(OldUpArrowBuiltin.on(new int[]{origW.ia+am}, new int[]{0}, origW, blame), w, 0, blame);
    }
    chk: {
